...
export async function signUp(credentials: SignUpValues): Promise<{ error: string }> {
  try {
    ...
    if (existingUserByEmail) {
      if (existingUserByEmail.hasSubscription) return { error: "Email already taken" };

      if (existingUserByEmail.username.toLowerCase() !== username.toLowerCase()) {
        return { error: "Account already exists. Please log in or use the original username." };
      }

      const checkoutRes = await fetch(`${process.env.NEXT_PUBLIC_BASE_URL}/api/stripe/create-checkout-session`, {
        method: "POST",
        body: JSON.stringify({ userId: existingUserByEmail.id, email }),
    ...
    if (existingUserByUsername) return { error: "Username already taken" };
