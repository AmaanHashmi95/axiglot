...
export async function POST(req: NextRequest) {
  ...
  const { videoId, sentenceIds, words, translations, audioUrl } = await req.json();
  ...
  await Promise.all(
    targetSentences.map(async (s) => {
      ...
      return prisma.subtitleTargetSentence.update({
        where: { id: s.id },
        data: {
          bookmarkedEnglish: translations.map((t: { text: string }) => t.text).join(" "),
          bookmarkedTransliteration: words.map((w: { transliteration?: string; text: string }) => w.transliteration || w.text).join(" "),
        },
      });
    })
  );
